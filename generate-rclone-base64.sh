...
# Validate the base64 string can be decoded back
if ! echo "$BASE64_CONFIG" | base64 -d >/dev/null 2>&1; then
    log ERROR "Generated base64 configuration is invalid and cannot be decoded."
    exit 1
fi

# Check minimum length (base64 config should be substantial)
if [ ${#BASE64_CONFIG} -lt 50 ]; then
    log WARN "Base64 configuration seems unusually short (${#BASE64_CONFIG} characters). Verify your rclone config is complete."
fi
